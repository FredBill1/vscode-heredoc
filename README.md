...
| 正文语言 | 诊断来源 | 隐藏文档下的行为 |
| --- | --- | --- |
| TypeScript、JavaScript | VS Code 内置 TypeScript 扩展的 `typescript.tsserverRequest` 命令，以及它主动发布的诊断 | 扩展启用时主动请求语法错误；若 `auto` 模式的内存 URI 不受支持，仅为诊断创建一次不可变文件快照重试。 |
| YAML | 已安装的 YAML 扩展主动发布的诊断 | 已验证可映射到 heredoc 正文。 |
| Python | Pylance 等已安装扩展主动发布的诊断 | Pylance 默认可能跳过没有编辑器标签页的隐藏文件，因此不能保证报错。 |
| Shell | Bash IDE 等已安装扩展主动发布的诊断 | 能否报错取决于扩展及其检查工具；例如 Bash IDE 的部分检查依赖 ShellCheck。 |
| 其他语言 | 对应扩展主动发布的诊断 | 取决于目标扩展是否支持该临时文档。 |

本扩展不会打开临时编辑器标签页、调用临时文档的保存操作、修改目标扩展的设置，或自行运行语言解析器。VS Code 没有通用的“向其他扩展请求诊断”接口，因此补全可用也不代表该扩展会检查隐藏文档。目标扩展缺席或不检查隐藏文档时，正文仍可获得语法高亮。
...
- 补全、悬停和定义取决于目标扩展是否安装、启用并支持临时文档。除 TypeScript/JavaScript 的专用命令外，诊断取决于目标扩展是否主动发布结果。部分扩展可能只对真实文件工作，此时可尝试 `documentMode: "file"`。
...
本机已安装 Python/Pylance、YAML 和 Bash IDE 时，可运行 `npm run test:live`。该测试在 `.vscode-test` 中创建独立配置目录，仅链接这些已安装扩展，并联调补全、TypeScript 与 YAML 的真实语法错误及诊断清除，同时记录 Pylance、Bash IDE 是否对隐藏文档发布诊断。可用 `HEREDOC_EXTENSIONS_DIR` 指定扩展安装目录；将 `HEREDOC_LIVE_ALL_EXTENSIONS=1` 时会使用该目录下的全部扩展，并在安装了 LimeGray 的环境中联调其活动主题颜色。目标扩展缺席时仍可用 `npm run test:host` 验证内置 Python grammar 可加载，支持高亮回退。
